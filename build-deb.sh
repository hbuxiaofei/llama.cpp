...
BUILD_ARCHIVE="llama-engin"

[ -d build ] && rm -rf build

[ ! -d build ] && mkdir build

cmake -B build -DGGML_RPC=ON -DLLAMA_CURL=ON || exit 1

cmake --build build --config Release -j $(nproc) --target rpc-server --target llama-server || exit 1
